...
//        self.navigationItem.leftBarButtonItem = suggestButton
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let challengeModel = self.currentChallengesObjects[indexPath.item]["challenge"] as! PFObject
        let currentChallengeData = self.currentChallengesObjects[indexPath.item] as! PFObject
        let currentCellContentArray:[[String]] = challengeModel["stepContent"] as! [[String]]
        let contentDictionary:[[String:String]] = self.contentDictionary(currentCellContentArray)
        ...
        if currentStepCount == 0{
            ...
            let subtitleString = reason[0] + ": " + reason[1]
            ...
            let currentStepTitle:String = currentStepTitles[currentStepCount - 1]
            let subtitleString = "Step \(currentStepCount): \(currentStepTitle)"
            cell.subtitleLabel.text = subtitleString
            cell.canvasTableView.reloadData()
        }
    ...
    func scrollViewDidEndDecelerating(scrollView: UIScrollView) {
        let currentPage:CGFloat = self.currentChallengesCardsCollectionView.contentOffset.x / self.currentChallengesCardsCollectionView.frame.size.width
    ...
    func loadCurrentChallenges(){
        ...
        let queryIsCurrentIsCurrentUser = PFQuery(className:"UserChallengeData")
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "addChallenge"{
            let modalVC = segue.destinationViewController as! UINavigationController
            let challengeGalleryVC = modalVC.childViewControllers[0] as! ChallengeGalleryViewController
    ...
    func nextStepButtonTapped(sender:UIButton!){
        ...
        let currentCell = cells[0] as! ChallengesTabCollectionViewCell
        ...
        for var section:Int = 0; section < currentCell.canvasTableView.numberOfSections; section++ {
            ...
            for var row:Int = 0; row < currentCell.canvasTableView.numberOfRowsInSection(section); row++ {
                ...
                if currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) is GallerySelectTableViewCell{
                    let cell = currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) as! GallerySelectTableViewCell
                    ...
                    let cell = currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) as! PromptAndAnswerTableViewCell
                    self.stepFullUserDataDictionary += cell.collectData()
                }
                else if currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) is PickerViewTableViewCell{
                    let cell = currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) as! PickerViewTableViewCell
                    self.stepFullUserDataDictionary += cell.collectData()
                }
                else if currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) is TextFieldInputTableViewCell{
                    let cell = currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) as! TextFieldInputTableViewCell
                    self.stepFullUserDataDictionary += cell.collectData()
                }
                else if currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) is BoolPickerTableViewCell{
                    let cell = currentCell.canvasTableView.cellForRowAtIndexPath(cellPath) as! BoolPickerTableViewCell
                    self.stepFullUserDataDictionary += cell.collectData()
                }
        ...
        let indexPath:NSIndexPath = self.currentChallengesCardsCollectionView.indexPathForCell(cells[0] as! ChallengesTabCollectionViewCell)!
        let itemNumber = indexPath.item
        let currentChallengeObject:PFObject = currentChallengesObjects[itemNumber] as! PFObject
        var currentStepCount = currentChallengeObject["currentStepCount"] as! Int
        let currentChallengeModel = currentChallengeObject["challenge"] as! PFObject
        ...
        if currentStepCount < currentChallengeModel["stepTitle"]!.count{
            ...
            currentChallengeObject.saveInBackgroundWithBlock{(succeeded, error) -> Void in
                if error == nil{
                    ...
                    let currentStepTitle:String = currentStepTitles[currentStepCount - 1]
                    let subtitleString:String = "Step \(currentStepCount): \(currentStepTitle)"
...
//            let minutes = components.minute
            ...
            let newActivity = PFObject(className: "Activity", dictionary: ["challenge":currentChallengeModel, "userChallengeData":currentChallengeObject, "ownerUser":PFUser.currentUser()!])
    ...
    func contentDictionary(raw:[[String]]) -> [[String:String]]{
        ...
        for contentArrayOfAStep in raw{
            ...
            while(indexCount < raw[arrayCount].count){
                let key = contentArrayOfAStep[indexCount]
                let value = contentArrayOfAStep[indexCount + 1]
